...
from src.xray.lau.peak_finding import find_all_peaks_naive


def test_find_all_peaks_naive_no_smoothing_lau():
    ...
    peaks = find_all_peaks_naive(df, threshold=0.1, distance=1, smoothing_window=0)
    assert len(peaks) == 4
    assert np.array_equal(peaks, [1, 4, 7, 10])


def test_find_all_peaks_naive_with_smoothing_lau():
    ...
    peaks_no_smooth = find_all_peaks_naive(df, threshold=0.5, distance=1, smoothing_window=0)
    ...
    peaks_with_smooth = find_all_peaks_naive(df, threshold=0.5, distance=1, smoothing_window=3)
...
def test_find_all_peaks_naive_smoothing_window_one_lau():
    # Smoothing window of 1 should behave like no smoothing
    data = {"Intensity": [0, 1, 0, 0, 5, 0, 0, 10, 0, 0, 2, 0], "Angle": range(12)}
    df = pd.DataFrame(data)

    peaks_no_smooth = find_all_peaks_naive(df, threshold=0.1, distance=1, smoothing_window=0)
    peaks_smooth_one = find_all_peaks_naive(df, threshold=0.1, distance=1, smoothing_window=1)

    assert np.array_equal(peaks_no_smooth, peaks_smooth_one)


def test_find_all_peaks_naive_smoothing_window_even_or_invalid_lau():
    ...
    peaks_no_smooth = find_all_peaks_naive(df, threshold=0.1, distance=1, smoothing_window=0)
    peaks_smooth_even = find_all_peaks_naive(df, threshold=0.1, distance=1, smoothing_window=2)
